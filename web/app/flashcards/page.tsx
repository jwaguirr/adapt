...
import { ChevronLeft, ChevronRight, Volume2, Settings, Shuffle, X, Check } from 'lucide-react';
...
export default function FlashcardGame(): React.ReactElement {
    ...
    const getButtonClass = (answer: string): string => {
        ...
        if (answer === currentCard.back) return 'bg-green-100 text-green-800 border-green-500';
        if (answer === selectedAnswer) return 'bg-red-100 text-red-800 border-red-500';
        return 'bg-white text-gray-500';
    };

    const progress: number = cards.length > 0 ? ((currentIndex + 1) / cards.length) * 100 : 0;
                ...
                <div className="flex flex-col items-center justify-center p-4" style={{minHeight: 'calc(100vh - 4rem)'}}>
                    <div className="w-full max-w-2xl text-center bg-white p-8 rounded-xl shadow-lg">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-left mx-auto max-w-xs mb-8">
                            <div className="bg-green-100 p-4 rounded-lg"><p className="text-xl font-semibold text-green-800">Correct</p><p className="text-4xl font-bold text-green-600">{correctCount}</p></div>
                            <div className="bg-red-100 p-4 rounded-lg"><p className="text-xl font-semibold text-red-800">Incorrect</p><p className="text-4xl font-bold text-red-600">{incorrectCount}</p></div>
                        </div>
                         <button onClick={handleRestart} className="w-full max-w-xs bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 transition-transform transform hover:scale-105">Play Again</button>
                    </div>
                </div>
            </div>
        )
                 ...
                 {cards.length > 0 && currentCard ? (
                    <div className="w-full max-w-2xl">
                        <div className="flex items-center justify-between mb-4">
                            <div className="w-full bg-gray-200 rounded-full h-2.5 mr-4"><div className="bg-green-500 h-2.5 rounded-full" style={{ width: `${progress}%` }}></div></div>
                            <span className="text-sm font-semibold text-gray-600 whitespace-nowrap">{currentIndex + 1} / {cards.length}</span>
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                            {answerOptions.map((answer, index) => (
                                <button key={index} onClick={() => handleSelectAnswer(answer)} disabled={selectedAnswer !== null} className={`flex items-center justify-between p-4 rounded-lg border-2 transition-all duration-300 transform hover:scale-105 ${getButtonClass(answer)}`}>
                                    <span className="font-semibold text-base text-left">{answer}</span>
                                    {selectedAnswer === answer && (isCorrect ? <Check className="text-green-600" /> : <X className="text-red-600" />)}
                                </button>
                            ))}
                        </div>
                        <div className="flex items-center justify-between mt-8">
                            ...
                            <div className="flex items-center space-x-4">
                                <button onClick={handlePrev} disabled={currentIndex === 0} className="p-3 rounded-full bg-white border shadow-sm disabled:opacity-50 hover:bg-gray-100"><ChevronLeft size={28} /></button>
                                <button onClick={handleNext} className="p-3 rounded-full bg-white border shadow-sm hover:bg-gray-100"><ChevronRight size={28} /></button>
                            </div>
                             <button onClick={handleRestart} className="text-gray-400 hover:text-gray-600"><Shuffle size={24} /></button>
                        </div>
                    </div>
                 ) : (
                    <div className="text-center">
                        <h2 className="text-2xl font-semibold text-gray-700">No phrases found.</h2>
                        <p className="text-gray-500 mt-2">Please add some phrases to your database to start playing.</p>
                    </div>
                 )}
            </div>
            <style jsx global>{`
